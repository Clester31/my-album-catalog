...
export default function YearlyCatalog({ catalog }: { catalog: CatalogItem[] }) {
    ...
    const [currentYearItemsByMonth, setCurrentYearItemsByMonth] = useState<CatalogItem[][]>(
        ...
        <div className="flex flex-col gap-4 h-full p-4 w-full overflow-y-hidden">
            <div className="flex flex-row gap-4 items-center text-center justify-center">
                <button onClick={() => changeYear(-1)}>
                    <i className="fa-solid fa-caret-left cursor-pointer text-2xl text-start hover:scale-110 hover:text-blue-800 transition-250 ease-in"></i>
                </button>
                <h1 className="text-2xl">{year}</h1>
                <button onClick={() => changeYear(1)}>
                    <i className="fa-solid fa-caret-right cursor-pointer text-2xl text-start hover:scale-110 hover:text-blue-800 transition-250 ease-in"></i>
                </button>
            </div>
                ...
                {currentYearItems.length > 0 ? (
                    <div className="flex flex-col gap-4">
                        {currentYearItemsByMonth.map((monthItems, idx) => (
                            <div
                                key={idx}
                                className={`flex flex-col items-start border-6 ${borderColors[idx]} p-4 rounded-4xl overflow-x-auto`}
                            >
                                ...
                                <div className="flex flex-row gap-2">
                                    {monthItems.length > 0 ? (
                                        ...
                                        <p className="text-sm text-gray-500">No albums cataloged for this month</p>
                                    )}
                                </div>
                            </div>
                        ))}
                    ...
                    <p className="text-xl text-center">No albums cataloged for this year</p>
                )}
            </div>
        </div>
    );
